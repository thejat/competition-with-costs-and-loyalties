...
def constraints_each_firm(p_strong_market,p_weak_market):
    if p_strong_market >= p_weak_market: return True
    return False

def compute_equilibrium(payoff_matrices,transition_prob_matrices,discount_factors,pa_arr,pb_arr,show_progress=True,plot_path=True):
...
def ml_get_metrics_theory(ca,cb,la,lb,F,f,deltaf):
...
def ml_get_metrics_computed(ca,cb,la,lb,F,f,deltaf,maxpx=10,npts=20,show_progress=False,plot_path=False):
    ...
    for i,ca in enumerate(ca_arr):
        if flag_theory:
            paa_arr[i],pab_arr[i],pbb_arr[i],pba_arr[i], xia_arr[i],xib_arr[i],vaao_arr[i],vabo_arr[i],vbbo_arr[i],vbao_arr[i]  \
                = ml_get_metrics_theory(ca,cb,la,lb,F,f,deltaf)
        else:
            paa_arr[i],pab_arr[i],pbb_arr[i],pba_arr[i], xia_arr[i],xib_arr[i],vaao_arr[i],vabo_arr[i],vbbo_arr[i],vbao_arr[i]  \
                = ml_get_metrics_computed(ca,cb,la,lb,F,f,deltaf,maxpx,npts,show_progress,plot_path)
...
def ll_constraints_market(p_strong_firm,p_weak_firm,c_strong_firm,c_weak_firm,l_strong_firm,s_strong_firm,dist='uniform'):
    ''' 
    let _strong_firm suffix represent the firm for which state a implies customer is in its strong market
    e.g., in state where cust is in B's strong mkt: pbb >= cb and pab >= ca and (0 <= pbb-pab) and  (pbb-pab <= lb) 
    '''
    if p_strong_firm >= c_strong_firm and \
        p_weak_firm >= c_weak_firm and \
        (s_strong_firm <= p_strong_firm-p_weak_firm) and  \
        (p_strong_firm-p_weak_firm <= l_strong_firm + s_strong_firm):
        return True
    return False
